...
export function renderBlock(block: Block, variables: Record<string, string>, isSelected = false): string {
  const sel = isSelected ? 'outline:2px solid #6366f1;outline-offset:3px;border-radius:2px;' : '';
  const v = (t: string) => escapeHtml(injectVariables(t, variables));
  const lines = (t: string) => v(t).split('\n').map((l) => l || '<br/>').join('<br/>');

  switch (block.type) {
    case 'contract_header': {
      const f = parseKV(block.content);
      const logo = f['logo'] || '';
      const logoHtml = logo
      ...
      return `<div style="${sel}margin:0 0 24px;border-bottom:3px solid #0f172a;padding-bottom:16px;">
        ...
        <div style="text-align:center;margin-top:14px;font-size:14pt;font-weight:800;text-transform:uppercase;letter-spacing:0.5px;">${v(f['titre'] || '')}</div>
      </div>`;
    }
    ...
    case 'definitions': {
      const rows = block.content.split('\n').filter((l) => l.includes(':')).map((l) => {
        const idx = l.indexOf(':');
        return { term: l.substring(0, idx).trim(), def: injectVariables(l.substring(idx + 1).trim(), variables) };
      });
      return `<div style="${sel}margin:14px 0;border:1px solid #e2e8f0;border-radius:6px;overflow:hidden;">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:9.5pt;">
          ${rows.map((r, i) => `<tr style="background:${i % 2 === 0 ? '#f8fafc' : 'white'};">
            <td style="padding:8px 14px;font-weight:700;width:28%;border-right:2px solid #e2e8f0;border-bottom:1px solid #f1f5f9;">« ${escapeHtml(r.term)} »</td>
            <td style="padding:8px 14px;color:#475569;border-bottom:1px solid #f1f5f9;line-height:1.6;">${escapeHtml(r.def)}</td>
          </tr>`).join('')}
        </table>
      </div>`;
    }
    ...
    case 'heading':
      return `<div style="${sel}font-size:14pt;font-weight:800;text-align:center;text-transform:uppercase;letter-spacing:0.5px;margin:0 0 18px;">${lines(block.content)}</div>`;

    case 'article':
      return `<div style="${sel}font-size:11pt;font-weight:700;margin:16px 0 6px;color:#0f172a;">${lines(block.content)}</div>`;
    ...
    case 'clause':
      return `<div style="${sel}font-size:10pt;line-height:1.75;margin:0 0 10px;text-align:justify;">${lines(block.content)}</div>`;

    case 'parties':
      return `<div style="${sel}font-size:10pt;line-height:1.75;margin:0 0 16px;padding:12px 16px;background:#f8fafc;border-left:3px solid #0f172a;">${lines(block.content)}</div>`;
      ...
      return `<div style="${sel}margin:14px 0;border:1px solid #d1fae5;border-radius:6px;overflow:hidden;">
        ...
        <div style="padding:12px 14px;background:#f0fdf4;">
          ...
          <table style="width:100%;border-collapse:collapse;font-size:10pt;">
            <tr style="border-bottom:1px solid #d1fae5;"><td style="padding:5px 0;color:#475569;">Montant HT</td><td style="text-align:right;font-weight:500;">${v(f['montant_ht'] || '')} €</td></tr>
            <tr style="border-bottom:1px solid #d1fae5;"><td style="padding:5px 0;color:#475569;">TVA (${f['tva_rate'] || '20'}%)</td><td style="text-align:right;font-weight:500;">${v(f['montant_tva'] || '')} €</td></tr>
            <tr><td style="padding:8px 0;font-size:11pt;font-weight:800;">Total TTC</td><td style="text-align:right;font-size:11pt;font-weight:800;color:#059669;">${v(f['montant_ttc'] || '')} €</td></tr>
          </table>
        </div>
      </div>`;
    }
    ...
    case 'form_fields': {
      ...
      return `<div style="${sel}margin:12px 0;padding:14px;border:1px solid #cbd5e1;border-radius:6px;">
        ${title ? `<div style="font-size:9pt;font-weight:700;margin-bottom:10px;text-transform:uppercase;letter-spacing:0.5px;">${escapeHtml(injectVariables(title, variables))}</div>` : ''}
        ${fields.map((f2) => `<div style="font-size:10pt;line-height:2.2;border-bottom:1px solid #e2e8f0;margin-bottom:4px;">${escapeHtml(injectVariables(f2, variables)).replaceAll('___', '<span style="display:inline-block;min-width:120px;border-bottom:1px solid #94a3b8;"> </span>')}</div>`).join('')}
      </div>`;
    }
    ...
    case 'checkbox_group': {
      ...
      return `<div style="${sel}margin:12px 0;padding:12px;border:1px solid #e2e8f0;border-radius:6px;">
        ${title ? `<div style="font-size:9pt;font-weight:700;margin-bottom:8px;text-transform:uppercase;letter-spacing:0.5px;">${escapeHtml(injectVariables(title, variables))}</div>` : ''}
        ${items.map((item) => `<div style="font-size:10pt;line-height:1.9;">${escapeHtml(injectVariables(item, variables)).replaceAll('☐', '<span style="display:inline-block;width:12px;height:12px;border:1px solid #475569;vertical-align:middle;margin-right:4px;"></span>')}</div>`).join('')}
      </div>`;
    }
      ...
      return `<div style="${sel}margin:12px 0;overflow:hidden;border-radius:6px;border:1px solid #e2e8f0;">
        <table style="width:100%;border-collapse:collapse;font-size:9.5pt;">
          <thead><tr style="background:#0f172a;color:white;">
            ${headers.map((h) => `<th style="padding:7px 10px;text-align:left;font-weight:600;">${escapeHtml(h.trim())}</th>`).join('')}
          </tr></thead>
          <tbody>${rows.map((row, i) => `<tr style="background:${i % 2 === 0 ? '#f8fafc' : 'white'};">
            ${row.map((cell) => `<td style="padding:7px 10px;border-top:1px solid #e2e8f0;">${escapeHtml(injectVariables(cell.trim(), variables))}</td>`).join('')}
          </tr>`).join('')}</tbody>
        </table>
      </div>`;
    }
    ...
    case 'signature_block':
      return `<div style="${sel}margin-top:28px;">
        ...
        <div style="display:flex;justify-content:space-between;gap:20px;margin-top:16px;">
          ${['Prestataire', 'Client', 'Conseiller'].map((s) => `<div style="flex:1;text-align:center;"><div style="height:50px;border-bottom:1px solid #1e293b;margin-bottom:6px;"></div><div style="font-size:8.5pt;color:#475569;">Signature du ${s}</div></div>`).join('')}
        </div>
      </div>`;

    case 'info_box':
      return `<div style="${sel}margin:12px 0;padding:12px 16px;background:#fffbeb;border:1px solid #fcd34d;border-radius:4px;font-size:9.5pt;line-height:1.65;">${lines(block.content)}</div>`;
    ...
    case 'retraction_form':
      return `<div style="${sel}margin:14px 0;border:1px solid #e2e8f0;border-radius:4px;padding:14px;background:#fafafa;">
        ...
        <div style="font-size:9pt;line-height:1.7;color:#475569;">${lines(block.content)}</div>
      </div>`;
    ...
    default:
      return block.content
        ? `<div style="${sel}font-size:10pt;line-height:1.6;margin-bottom:8px;">${lines(block.content)}</div>`
        : '';
  }
}

export function renderContractToHtml(content: string, variables: Record<string, string>): string {
  const parsed = JSON.parse(content);
  const blocks: Block[] = parsed.blocks || [];
  const body = blocks.map((b) => `<div style="page-break-inside:avoid;">${renderBlock(b, variables)}</div>`).join('');
